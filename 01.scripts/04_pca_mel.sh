...
BAM_LIST=../../02.info/mel.bam.list

#this is the input file for the pca
INPUT=../../03.saf.maf.gl.all/mel/all_maf"$MIN_MAF_MEL"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH"_downsample"$DOWNSAMPLING_PERC".beagle.gz

echo "analyse covariance matrix on all individuals"
python3 ~/programmes/pcangsd/pcangsd.py \
	-beagle $INPUT -o ../../04.pca/mel/all_maf"$MIN_MAF_MEL"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH"_downsample"$DOWNSAMPLING_PERC"

echo "transform covariance matrix into PCA"
Rscript ../../01.scripts/Rscripts/make_pca_mel_maxdepth.R "$MIN_MAF_MEL" "$PERCENT_IND" "$BAM_LIST" "$MAX_DEPTH" "$DOWNSAMPLING_PERC"
