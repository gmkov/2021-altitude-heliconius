...
cov_mat<-npyLoad(file=paste0("../../04.pca/era/all_maf",MIN_MAF_ERA,"_maxdepth",MAX_DEPTH,"_downsample",DOWNSAMPLING_PERC,".cov.npy"))

#cov_mat<-as.matrix(read.table(paste0("../../04.pca/era/all_maf",MIN_MAF_ERA,"_maxdepth",MAX_DEPTH,"_downsample",DOWNSAMPLING_PERC,".cov"), header=F))
pca<-prcomp(cov_mat)
print(summary(pca))
...
#plot pca
pdf(file=paste0("../../04.pca/era/all_maf",MIN_MAF_ERA,"_maxdepth",MAX_DEPTH,"_downsample",DOWNSAMPLING_PERC,".pca.pdf"))
par(mfrow=c(1,1))
plot(pca$x[,1], pca$x[,2], pch=20, ylab="PC2", xlab="PC1")
#plot(pca$x[,3], pca$x[,4], pch=20, ylab="PC4", xlab="PC3")
dev.off()

write.table(pca$x, paste0("../../04.pca/era/all_maf",MIN_MAF_ERA,"_maxdepth",MAX_DEPTH,"_downsample",DOWNSAMPLING_PERC,".pca"), quote=F)
