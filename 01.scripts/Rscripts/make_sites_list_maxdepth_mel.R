...
maf<-read.table( paste0("../../03.saf.maf.gl.all/mel/all_maf",MIN_MAF,"_pctind",PERCENT_IND,"_maxdepth",MAX_DEPTH_FACTOR,"_downsample",DOWNSAMPLING_PERC,".mafs"), header=T)
head (maf)
...
write.table(sites_order, paste0("../../02.info/sites_mel_maf", MIN_MAF, "_pctind",PERCENT_IND,"_maxdepth",MAX_DEPTH_FACTOR,"_downsample",DOWNSAMPLING_PERC,"_minMAC2"), row.names=F, col.names=F, sep="\t", quote=F)
write.table(regions_order, paste0("../../02.info/regions_mel_maf", MIN_MAF, "_pctind",PERCENT_IND,"_maxdepth",MAX_DEPTH_FACTOR,"_downsample",DOWNSAMPLING_PERC,"_minMAC2"), row.names=F, col.names=F, sep="\t", quote=F)
