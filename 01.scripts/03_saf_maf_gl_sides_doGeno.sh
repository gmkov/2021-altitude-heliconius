...
-minInd $MIN_IND -minMaf ${!MIN_MAF} -setMaxDepth $MAX_DEPTH -downSample $DOWNSAMPLING_PERC \
-b ../../02.info/"$SP"."$SIDE".bam.list \
-out ../../03.saf.maf.gl.all/era/"$SIDE"_maf"${!MIN_MAF}"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH"_downsample"$DOWNSAMPLING_PERC"
...
gunzip /home/mgm49/rds/rds-cj107-jiggins-rds/projects/mgm49/era.mel.altitude/03.saf.maf.gl.all/"$SP"/"$SIDE"/"$SP"."$SIDE"_maf"${!MIN_MAF}"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH"_downsample"$DOWNSAMPLING_PERC".mafs.gz 
Rscript /home/mgm49/rds/rds-cj107-jiggins-rds/projects/mgm49/era.mel.altitude/01.scripts/Rscripts/make_sites_list_maxdepth_sides.R "${!MIN_MAF}" "$PERCENT_IND" "$MAX_DEPTH" "$DOWNSAMPLING_PERC" "$SP" "$SIDE"
angsd sites index ../../02.info/sites_"$SP"_"$SIDE"_maf"${!MIN_MAF}"_pctind"$PERCENT_IND"_maxdepth"$MAX_DEPTH"_downsample"$DOWNSAMPLING_PERC"
